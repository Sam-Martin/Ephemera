...
exports.handler = function(event, context, params) {
    ...
    s3.getObject({Bucket: config.bucketName, 'Key':event.key}, function(err,getData){
        
        if (err) context.fail("Error loading object" + JSON.stringify(err.stack))
    	
    	// Delete the object before we return its contnets
    	s3.deleteObject({Bucket: config.bucketName, 'Key':event.key}, function(err,deleteData){

    		if (err) context.fail("Error deleting object" + JSON.stringify(err.stack))
